...
public class Plague : MonoBehaviour
{
    ...
    protected MainRoadEnemy target;
    ...
    private void FindTarget()
    {
        ...
        if(hits.Length > 0){
            ...
            {
                target = hit.transform.GetComponent<MainRoadEnemy>();
                target.SetSpeed(speedMultiplier, slowTime);
                Debug.Log(target.name + " slowed!");
                if(level >= 2)
                {
                    target.SetDamageAmplifier(damageAmplifier, damageAmplifierTime);
